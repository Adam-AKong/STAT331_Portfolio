...
For my reflection I changed several parts of the problems that Professor Theobold indicated was incorrect and could be improved on. Even on my challenge I made changes even though I do not need to resubmit the challenge. First thing I changed in my file was to change was hiding the messages or warning by adding the #\| message: FALSE. This hides the message output caused by library(tidyverse) and library(here). The second major change I made was to the boxplot question of the sections. The mistake I made was faceting questions 8 through 14, which I interpreted side-by-side boxplots as facet since we did that in the previous step. I corrected first by getting rid of hindfoot_length since we no longer needed that for the boxplot. I then set the y to species since we were looking for weight distributions in species and correctly placed several boxplots on the same plot. Here is something the professor Theobold mentioned in her comments but I decided to put the boxplot and keep the boxplot on the top. Here is my justification which is if I place the boxplot below the scatterplot points, then no one can see the boxplot and it would be useless to keep since there are so many dots on the graph. I think that the boxplot should be on top, just maybe a little transparency could be used to see the full distributions but I feel that adding a transparency is similar to redudant information considering the jitter shows how many points are plotted behind it already from the top and bottom. I placed the transparency since some plots may not have points on every gram, but even then seeing the points behind the boxplot does not tell much information about the distribution. Finally, I removed the y label for the boxplot graphs and found the correct method of flipping the boxplots from horizontal to vertical. Additionally I searched up a way to angle the x label for the flip boxplots since the names were so crowded against each other.

Overall I learned that I should be reading instructions more carefully to understand that boxplots side by side meant one graph. I also learned that using several functions to make the html and graphs nicer in general make everything look better. I think a lot of what I learned from this is how I can distribute boxplots so the graphs can have nicer readability and how readers should not have a harder time reading the html and graphs. I should learn to do things the newer and updated methods since outdated ones are inefficent compared to newer methods, especially with coordflip because switching x and y is way easier than assigning it to a variable and doing + coordflip() to it. Overall what I gained from the re submission is how do I make data more readable to normal readers.
...
The dimensions of the dataset is 30,463 rows and 15 columns which I found with the dim() function.
...
The str(surveys)list the three variable types which is character, date and a numerical data type. Although the give.attr at the bottom of the output gives more details, because it includes the specific numerical data variable as a double data type variable.
...
Once you have completed your lab you should upload your rendered HTML file to Canvas.
