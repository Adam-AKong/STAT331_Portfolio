...
sing_day <- function(dataset, line, phrase_col){
  ...
  if ({{ line }} == 1) {
    final_phrase <- glue(intro, str_c(phrases[1], "."),
                         "\n",
                         .sep = "\n")
  return(final_phrase)
  } else if ({{ line }} == 2) {
    final_phrase <- glue(intro, 
                         str_c(phrases[2], ", and"), 
                         str_c(phrases[1], "."), 
                         "\n",
                         .sep = "\n")
  return(final_phrase)
    ...
    final_phrase <- glue(intro, 
                         str_c(str_flatten(phrases[{{ line }}:3],
                                           collapse = "\n"),
                               .sep = ","), 
                         str_c(phrases[2],", and"), 
                         str_c(phrases[1],"."), 
...
```{r}
xmas2 |>
  sing_day(line = 9, phrase_col = Full.Phrase)
...
Okay, so I know I am not to use for loop for this step, but I am running out of time and running out of ideas how this function will work. I will go into office hours for some help with this function since my test function works individually but I can't figure out where my problem is for the function. It possibly is the return function where I output a cat function, but I'm planning to discuss with professor or someone else from the class what my issue is since I felt as if I spent too long looking at this. I can't currently grasp the problem with the issue with my output since individually it returns a string fine but I do want to move onto my re submissions.
...
map_chr(.x = xmas2$Day, ~sing_day(xmas2, .x, Full.Phrase))
